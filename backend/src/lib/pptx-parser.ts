// Server-side PPTX parser — same extraction logic as the frontend's
// src/lib/sft-training/pptx-parser.ts, ported to Node so we can parse the
// deck once on the server and hand the browser a small JSON payload instead
// of making it download the full (often multi-MB, image-heavy) .pptx file
// just to read a few KB of text.
  ...
  const slides = await Promise.all(
    slideFiles.map(async (slidePath, i) => {
      ...
      if (zip.files[notesPath]) {
        ...
        const notesParas = extractParagraphs(notesObj).filter((p) => !/^\d+$/.test(p.trim()));
